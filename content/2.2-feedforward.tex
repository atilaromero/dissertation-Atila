...
Assuming the input of the neural network uses one-hot encoding, categorical cross entropy is $-log(\hat{y}_i)$, where $\hat{y}_i$ is the neural network output for the predicted class $i$. Binary cross entropy is similar, but penalizes errors in all classes instead of only the predicted one. Mean squared error, like binary cross-entropy, also consider all classes simultaneously, but with a different formula. It is the mean of the component values of the square of the difference between the correct and predicted output vectors, $\frac{(\vec{y}-\vec{\hat{y}})^2}{N}$, where $y$ is the correct vector and $N$ is the number of dimensions (features) of the vector.
