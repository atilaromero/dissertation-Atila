...
The first error source, E1, is the focus of this chapter. In the file fragment classification task, it is expected that some portion of the classifications errors of a given model be caused by its inability to recognize patterns in the data if the data has high entropy. In these situations, even humans may be unable to tell if a sample is valid or if it is random data. It may happen that a search for an alternative model lead to better results, but the chances are low, as this type of error is hard to mitigate. 
...
% \levelC{Hypothesis}
The goal of the experiment of this chapter is to test the \textbf{hypothesis that part of the errors of file fragment classification can be explained by the existence of data that has no structure detectable by the model}. Not all file types have this kind of data, and on those that do, they should compose only a portion of the file.
